...
<head>
    <?php if (!empty($data) && isset($data['name']) && !empty($data['name'])): ?>
        <title>Ответы на опрос «<?= htmlspecialchars($data['name']) ?>» — Школа Менеджмента STRATOPLAN</title>
    <?php else: ?>
   ...
   <link href="scripts/styles/styles.css" rel="stylesheet" type="text/css">
    <link rel="shortcut icon" href="https://stratoplan-school.com/wp-content/uploads/2021/03/fav32.png" type="image/x-icon" />
    <link rel="apple-touch-icon" href="https://stratoplan-school.com/wp-content/uploads/2021/03/fav180.png" />
</head>
<body>
    <?php
    $name = $_POST['name'] ?? '';
